...
const arithmetic = (source: string) => {
  const tokens = source.match(/\d+(?:\.\d+)?|[()+\-*/]/g) ?? []; let cursor=0;
  const atom=():number=>{const token=tokens[cursor++];if(token==="("){const value=sum();if(tokens[cursor++]!==")")throw new Error("parenthesis");return value}if(token==="-")return-atom();const value=Number(token);if(!Number.isFinite(value))throw new Error("number");return value};
  const product=()=>{let value=atom();while(tokens[cursor]==="*"||tokens[cursor]==="/"){const op=tokens[cursor++];const right=atom();value=op==="*"?value*right:value/right}return value};
  const sum=()=>{let value=product();while(tokens[cursor]==="+"||tokens[cursor]==="-"){const op=tokens[cursor++];const right=product();value=op==="+"?value+right:value-right}return value};
  const result=sum(); return cursor===tokens.length&&Number.isFinite(result)?result:null;
};
const computedRows = (properties:any[], rows:any[]) => {
  const byId=new Map(rows.map(row=>[row.id,row]));
  return rows.map(row=>{const values={...row.values};for(const property of properties){if(property.type==="formula"){const expression=String(property.config.expression??"");const expanded=expression.replace(/\{([^}]+)\}/g,(_match,id)=>String(Number(values[id])||0));values[property.id]=arithmetic(expanded)}if(property.type==="rollup"){const relationIds=values[property.config.relationPropertyId];const selected=(Array.isArray(relationIds)?relationIds:[]).map(id=>byId.get(id)).filter(Boolean);const collected=selected.map(item=>item.values[property.config.targetPropertyId]).filter((value:any)=>value!==null&&value!==undefined&&value!=="");switch(property.config.aggregation){case"sum":values[property.id]=collected.reduce((total:any,value:any)=>total+(Number(value)||0),0);break;case"average":values[property.id]=collected.length?collected.reduce((total:any,value:any)=>total+(Number(value)||0),0)/collected.length:null;break;case"min":values[property.id]=collected.length?Math.min(...collected.map(Number)):null;break;case"max":values[property.id]=collected.length?Math.max(...collected.map(Number)):null;break;case"count":values[property.id]=collected.length;break;default:values[property.id]=collected.join(", ")}}}return{...row,values}});
};
const loadDatabase = async (id: string) => {
  ...
  return { ...database, properties, views, rows: computedRows(properties,rows) };
};
...
export async function registerDatabaseRoutes(app: FastifyInstance) {
  ...
  app.post("/api/v1/databases/:id/rows", { preHandler: authenticate }, async (request,reply)=>{const {id}=request.params as any;if(!await databaseAccess(request.user.id,id,true))return reply.code(403).send({error:"Forbidden"});const b=parse(databaseRowSchema,request.body,reply);if(!b)return;const [database]=await sql<{workspaceId:string}[]>`SELECT workspace_id FROM databases WHERE id=${id}`;const [titleProperty]=await sql<{id:string}[]>`SELECT id FROM database_properties WHERE database_id=${id} AND type='title' LIMIT 1`;const title=String(titleProperty?b.values[titleProperty.id]??"Untitled":"Untitled");const pageId=randomUUID();const rows=await sql.begin(async tx=>{await tx`INSERT INTO pages(id,workspace_id,title,position,created_by) VALUES(${pageId},${database!.workspaceId},${title},COALESCE((SELECT max(position)+1 FROM pages WHERE workspace_id=${database!.workspaceId}),0),${request.user.id})`;return tx`INSERT INTO database_rows(database_id,page_id,values,position,created_by) VALUES(${id},${pageId},${sql.json(b.values as any)},${b.position??Date.now()},${request.user.id}) RETURNING *`});return reply.code(201).send({row:rows[0]});});
  app.post("/api/v1/database-rows/:id/page", { preHandler: authenticate }, async(request,reply)=>{const{id}=request.params as any;const[row]=await sql<any[]>`SELECT r.*,d.workspace_id FROM database_rows r JOIN databases d ON d.id=r.database_id WHERE r.id=${id}`;if(!row||!await databaseAccess(request.user.id,row.databaseId,true))return reply.code(403).send({error:"Forbidden"});if(row.pageId)return{pageId:row.pageId};const[titleProperty]=await sql<{id:string}[]>`SELECT id FROM database_properties WHERE database_id=${row.databaseId} AND type='title' LIMIT 1`;const title=String(titleProperty?row.values[titleProperty.id]??"Untitled":"Untitled");const pageId=randomUUID();await sql.begin(async tx=>{await tx`INSERT INTO pages(id,workspace_id,title,position,created_by) VALUES(${pageId},${row.workspaceId},${title},COALESCE((SELECT max(position)+1 FROM pages WHERE workspace_id=${row.workspaceId}),0),${request.user.id})`;await tx`UPDATE database_rows SET page_id=${pageId} WHERE id=${id}`});return reply.code(201).send({pageId});});
  app.patch("/api/v1/database-rows/:id", { preHandler: authenticate }, async (request,reply)=>{const {id}=request.params as any;const [row]=await sql<any[]>`SELECT * FROM database_rows WHERE id=${id}`;if(!row||!await databaseAccess(request.user.id,row.databaseId,true))return reply.code(403).send({error:"Forbidden"});const b=parse(databaseRowSchema,request.body,reply);if(!b)return;const merged={...row.values,...b.values};await sql`UPDATE database_rows SET values=${sql.json(merged)},position=COALESCE(${b.position??null},position),updated_at=now() WHERE id=${id}`;const[titleProperty]=await sql<{id:string}[]>`SELECT id FROM database_properties WHERE database_id=${row.databaseId} AND type='title' LIMIT 1`;if(row.pageId&&titleProperty&&b.values[titleProperty.id]!==undefined)await sql`UPDATE pages SET title=${String(b.values[titleProperty.id]??"Untitled")},revision=revision+1,updated_at=now() WHERE id=${row.pageId}`;await runAutomations(row.databaseId,id,row.values,merged);const loaded=await loadDatabase(row.databaseId);return {row:(loaded as any).rows.find((item:any)=>item.id===id)};});
  app.delete("/api/v1/database-rows/:id", { preHandler: authenticate }, async (request,reply)=>{const {id}=request.params as any;const [row]=await sql<{databaseId:string;pageId:string|null}[]>`SELECT database_id,page_id FROM database_rows WHERE id=${id}`;if(!row||!await databaseAccess(request.user.id,row.databaseId,true))return reply.code(403).send({error:"Forbidden"});await sql.begin(async tx=>{await tx`DELETE FROM database_rows WHERE id=${id}`;if(row.pageId)await tx`DELETE FROM pages WHERE id=${row.pageId}`});return reply.code(204).send();});
